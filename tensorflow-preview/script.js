...
async function getData() {
    const productsDataReq = await fetch('http://localhost/tensorflow-preview/produtos/banana.json');  
...
function convertToTensor(data) {
    ...
    return tf.tidy(() => {
      ...
      const inputs = data.map(d => d.valor_unitario)
      const labels = data.map(d => d.quantidade);
...
async function trainModel(model, inputs, labels) {
  ...
  function testModel(model, inputData, normalizationData) {
    ...
    const originalPoints = inputData.map(d => ({
      x: d.valor_unitario, y: d.quantidade,
    }));
...
async function run() {
    ...
    tfvis.render.scatterplot(
      {name: 'Quantidade v Valor Unitário'},
      {values}, 
      {
        xLabel: 'Quantidade',
